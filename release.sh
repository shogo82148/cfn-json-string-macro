...
while read -r REGION; do
    ...
    aws s3 cp --region "$REGION" "$DIST/$REGION.yaml" "s3://$BUCKET/cfn-json-string-macro/latest.yaml"
    aws s3 cp --region "$REGION" "$DIST/$REGION.yaml" "s3://$BUCKET/cfn-json-string-macro/v$MAJOR.$MINOR.$PATCH.yaml"
    aws s3 cp --region "$REGION" "$DIST/$REGION.yaml" "s3://$BUCKET/cfn-json-string-macro/v$MAJOR.$MINOR.yaml"
    aws s3 cp --region "$REGION" "$DIST/$REGION.yaml" "s3://$BUCKET/cfn-json-string-macro/v$MAJOR.yaml"
done << EOS
af-south-1
ap-east-1
ap-northeast-1
